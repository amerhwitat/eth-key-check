...
def X(c,d=False):
 ...
 return p.wait()
def main():
 a=argparse.ArgumentParser();a.add_argument('--dry-run',action='store_true');a.add_argument('--only',choices=['all','python','java','node','native','sql'],default='all');a.add_argument('--onefile',action='store_true');a.add_argument('--python');a=a.parse_args();print(f'[BUILD] {R.name} {platform.system()} {platform.machine()}')
